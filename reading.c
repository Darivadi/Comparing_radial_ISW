...
int read_parameters( char filename[] )
{
  ...
  nread = fscanf(file, "%s", GV.FILENAMESIM);
...
int read_binary(void)
{
  ...
  /****** Simulation ******/
  inFile = fopen(GV.FILENAMESIM, "r");

  printf("Reading parameters from simulation\n");
  ...
  /*+++++ Memory allocation +++++*/
  gps = (struct grid *) calloc((size_t) GV.NTOTALCELLS, sizeof(struct grid));
  printf("Memory allocated!\n");
  printf("--------------------------------------------------\n");
  ...
  for(i=0; i<GV.NTOTALCELLS; i++)
    {
      /*..... File app2 .....*/
      nread = fread(&gps[i].potDot, sizeof(double), 1, inFile);
  ...
  printf("Data read for simulation!\n");
  fclose(inFile);
...
int write_binary()
{
  ...
  /*+++++ Reconstruction 1 +++++*/
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Rec1.bin", "w");
  ...
  /*+++++ Simulation +++++*/
#ifdef COMPLETERAY
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_complete.bin", "w");
#endif  

#ifdef UNTIL90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_until90Mpc.bin", "w");
#endif  

#ifdef FROM90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_from90Mpc.bin", "w");
#endif  
  ...
  for(m=0; m<GV.NRays; m++)
    {
      fwrite(&(ray_s[m].rad),      sizeof(double), 1, outFile);
      fwrite(&(ray_s[m].theta),    sizeof(double), 1, outFile);
      fwrite(&(ray_s[m].phi),      sizeof(double), 1, outFile);
      fwrite(&(ray_s[m].ISW_temp), sizeof(double), 1, outFile);
    }//for m                                                                                                    
...
int write_ascii(void)
{
  ...
  /*+++++ Reconstruction1 +++++*/
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Rec1.dat", "w");  
  ...
  for(m=0; m<GV.NRays; m++)
    {
      fprintf(outFile, "%16.8lf %16.8lf %16.8lf %16.8lf\n", 
	      ray_r1[m].rad, ray_r1[m].theta, ray_r1[m].phi, ray_r1[m].ISW_temp);
    }//for m
  ...
  /*+++++ Reconstruction2 +++++*/
#ifdef COMPLETERAY
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Rec2_complete.dat", "w");
#endif  
  
#ifdef UNTIL90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Rec2_until90Mpc.dat", "w");
#endif  
  
#ifdef FROM90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Rec2_from90Mpc.dat", "w");
#endif  
  ...
  for(m=0; m<GV.NRays; m++)
    {
      fprintf(outFile, "%16.8lf %16.8lf %16.8lf %16.8lf\n", 
	      ray_r2[m].rad, ray_r2[m].theta, ray_r2[m].phi, ray_r2[m].ISW_temp);
    }//for m
...
/*+++++ Simulation +++++*/
#ifdef COMPLETERAY
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_complete.dat", "w");
#endif  

#ifdef UNTIL90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_until90Mpc.dat", "w");
#endif  

#ifdef FROM90MPC
  outFile = fopen("./../../Processed_data/ISW_radial_app2_Sim_from90Mpc.dat", "w");
#endif  
  ...
  for(m=0; m<GV.NRays; m++)
    {
      fprintf(outFile, "%16.8lf %16.8lf %16.8lf %16.8lf\n", 
	      ray_s[m].rad, ray_s[m].theta, ray_s[m].phi, ray_s[m].ISW_temp);
    }//for m
