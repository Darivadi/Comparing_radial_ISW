...
int rand_rays_coordinates(void)
{
  ...
  for(m=0; m<GV.NRays; m++)
    {     
      ...
      ray_s[m].theta = ray_r1[m].theta; //Polar
      ray_s[m].phi   = ray_r1[m].phi; //Azimuth
      ...
      ray_s[m].vec_ini[X] = 0.0;
      ray_s[m].vec_ini[Y] = 0.0;
      ray_s[m].vec_ini[Z] = 0.0;
      ...
      ray_s[m].vec_end[X] = ray_s[m].rad * sin(ray_s[m].theta) * cos(ray_s[m].phi);
      ray_s[m].vec_end[Y] = ray_s[m].rad * sin(ray_s[m].theta) * sin(ray_s[m].phi);
      ray_s[m].vec_end[Z] = ray_s[m].rad * cos(ray_s[m].theta);
